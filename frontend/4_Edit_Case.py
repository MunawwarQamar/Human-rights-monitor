...
def show():
    ...
    with st.form("edit_case_form"):
        ...
        priority = st.selectbox(
            "Priority",
            ["low", "medium", "high"],
            index=["low", "medium", "high"].index(case.get("priority", "medium"))
        )

        status = st.selectbox(
            "Status",
            ["new", "under_investigation", "resolved", "archived"],
            index=["new", "under_investigation", "resolved", "archived"].index(case.get("status", "new"))
        )
        ...
        # ✅ Handle coordinates safely (list or dict)
        coords_raw = location.get("coordinates", [0.0, 0.0])
        if isinstance(coords_raw, dict):
            coords = coords_raw.get("coordinates", [0.0, 0.0])
